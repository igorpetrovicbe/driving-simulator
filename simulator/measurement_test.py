...
from image_h5_dataset import ImageDataset
import cv2
import os
import numpy as np
import itertools
...
dataset_path = "H:\\PycharmProjects\\self_driving_reader\\driving_images_test.h5"
print(dataset_path)
dataset = ImageDataset(h5_file_path=dataset_path)
...
if __name__ == "__main__":
    for sample_index in range(sample_size):
        ...
        with torch.no_grad():
            ...
            input_image_detached = dataset[random_idx].detach().cpu().numpy()

            input_image_detached = np.transpose(input_image_detached, (1, 2, 0))
            input_image_detached = input_image_detached.astype(np.uint8)
